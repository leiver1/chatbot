...
const ChatBot = () => {
  ...
  const [chats, setChats] = useState<{ role: string; icon: string; text: string }[]>([]);
  const [content, setContent] = useState<{ role: string; icon: string; text: string }>({
    role: "",
  ...
  const handleResponse = (): void => {
    if (content.text) {
      const userMessage = content.text.trim().toLowerCase();  
  ...
  return (
    <>
      ...
      >
        ...
        <Box sx={{ width: { xs: '100%', sm: '95%', md: '80%' }, p: { xs: 0, sm: 2 }, position: "relative" }}>
