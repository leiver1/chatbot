...
export default function ResponsiveDrawer({ children }: {children: ReactNode}) {
  ...
  // const [drawerWidth, setDrawerWidth] = useState(window.innerWidth < 400 ? 270 : 70)
  const [drawerWidth, setDrawerWidth] = useState(window.innerWidth < 800 ? 270 : 70);
          ...
          <List >
            <ListItem   sx={{  width: '100%', display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <IconButton onClick={()=>setIsLocked(!isLocked)}>
                <Icon fontSize={20} icon={isLocked ? 'mdi:lock-outline' : 'mdi:lock-open-outline'}></Icon>
              </IconButton>
            </ListItem>
            ...
            {drawerLists.map((item, index) => (
              <ListItem disableGutters key={index}>
                <ListItemButton
                  sx={{ borderRadius: 3, mx: 1, backgroundColor: isActive(item.path) && isDark ? '#121212' : isActive(item.path) && !isDark ? '#cfcfcf' : 'transparent'  }}
                  component={Link}
                  to={item.path}
                ...
                >
                  ...
                  <ListItemText sx={{overflow: 'hidden', whiteSpace: 'nowrap'}} primary={item.name}></ListItemText>
                </ListItemButton>
              </ListItem>
            ))}
          </List>
    ...
    <>
      <Dialog open={open} fullWidth maxWidth="sm">
        ...
        <DialogContentText>
          ...
          Sie sind dabei den Chatvelauf mit dem Chatbot permanent zu löschen <br /> Sind Sie sich sicher?
        </DialogContentText>
        </DialogContent>
        <DialogActions>
        <Button onClick={()=> setOpen(false)} variant="text" sx={{width: '70px'}}color="secondary">Nein</Button>
        <Button disableElevation onClick={deleteChat} sx={{width: '70px'}} variant="contained" color="error">Ja</Button>
        </DialogActions>
      </Dialog>
    </>
  )
...
<Box>
          ...
          <IconButton onClick={()=>dispatch(handleTheme())}>
            <Icon icon={isDark ? 'mdi:moon-and-stars': 'mdi:weather-sunny' }></Icon>
          </IconButton>
